    ...
    await $.post("/billett/hentForekomsterDatoTid/", forekomstDato).done((res) => {
       ...
       let  html = '<div class="container">';
        ...
        for (i = 0; i < res.length; i++) {
            ...
            html += ' <button type="button" class="btn btn-success" id="tidspunkt' + i + '" value="' + res[i].avgangsTid + '" > Velg </button > '
            html += "</p> ";
            html += "    </div>"
            html += " </div>";
        }
...
async function hentKjoretoyInfo() {
    ...
    await $.get("/billett/hentKjoretoyInfo/").done((res) => {
        ...
        let uniqueKjoretoy = kjoretoyInfo.reduce((result, kjoretoy) => {
            ...
            let found = result.reduce((end, res) => res.typeKjoretoy == kjoretoy.typeKjoretoy | end, false);
